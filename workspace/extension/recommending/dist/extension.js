...
function addDependency(pomPath, gid, aid, version) {
    return __awaiter(this, void 0, void 0, function* () {
        // Find out <dependencies> node and insert content.
        const contentBuf = yield fse.readFile(pomPath);
        const projectNodes = lexerUtils_1.getNodesByTag(contentBuf.toString(), lexerUtils_1.XmlTagName.Project);
        if (projectNodes === undefined || projectNodes.length !== 1) {
            throw console.log("Only support POM file with single <project> node.");
        }
        const projectNode = projectNodes[0];
        const dependenciesNode = projectNode.children && projectNode.children.find(node => node.tag === lexerUtils_1.XmlTagName.Dependencies);
        if (dependenciesNode !== undefined) {
            yield insertDependency(pomPath, dependenciesNode, gid, aid, version);
        }
        else {
            yield insertDependency(pomPath, projectNode, gid, aid, version);
        }
    });
}
function insertDependency(pomPath, targetNode, gid, aid, version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const eol = currentDocument.eol === vscode.EndOfLine.LF ? "\n" : "\r\n";
        let insertPosition;
        let targetText;
        if (targetNode.tag === lexerUtils_1.XmlTagName.Dependencies) {
        ...
        else if (targetNode.tag === lexerUtils_1.XmlTagName.Project) {
        ...
        const endingPosition = currentDocument.positionAt(currentDocument.offsetAt(insertPosition) + targetText.length);
        textEditor.revealRange(new vscode.Range(insertPosition, endingPosition));
    });
}
function constructDependencyNode(gid, aid, version, baseIndent, indent, eol) {
    return [
        eol,
        "<dependency>",
...
function constructDependenciesNode(gid, aid, version, baseIndent, indent, eol) {
    return [
        eol,
        "<dependencies>",
        `${indent}<dependency>`,
...
(function (XmlTagName) {
    ...
    XmlTagName["ArtifactId"] = "artifactId";
    XmlTagName["Version"] = "version";
    XmlTagName["Dependencies"] = "dependencies";
    XmlTagName["Plugins"] = "plugins";
    XmlTagName["Project"] = "project";
})(XmlTagName = exports.XmlTagName || (exports.XmlTagName = {}));
class ElementNode {
...
function getElementHierarchy(text, tokens, tagOrOffset) {
    ...
    while (i < n) {
        ...
        const currentNode = elementNodes[elementNodes.length - 1];
        switch (token[0]) {
            case xml_zero_lexer_1.NodeTypes.XML_DECLARATION:
            case xml_zero_lexer_1.NodeTypes.ELEMENT_NODE: {
                // [_type, start, end] = token;
                const [start, end] = token.slice(1, 3);
                const newElement = new ElementNode(currentNode, text.substring(start, end));
            ...
            case xml_zero_lexer_1.NodeTypes.CLOSE_ELEMENT: {
...
function getArtifacts(keywords) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const raw = yield httpsGet(`${URL_BASIC_SEARCH}?${toQueryString(params)}`);
        try {
...
function getLatestVersion(gid, aid) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const params = {
                q: `g:"${gid}" AND a:"${aid}"`,
...
function httpsGet(urlString) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
            let result = "";
            https.get(url.parse(urlString), (res) => {
                res.on("data", chunk => {
